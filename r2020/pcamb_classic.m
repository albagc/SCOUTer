...
arguments
    X double
    ncomp double {mustBeInRangeSize(ncomp, X, 2, 0)}
...
function mustBeInRangeSize(arg,B,dimB, liminf)
    if (arg < liminf) || (arg > size(B,dimB))
        error(['Value assigned to Data is not in range ',...
            num2str(liminf),'...',num2str(size(B,dimB))])
    end
end